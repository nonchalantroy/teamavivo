...
```
```{r replace spaces ,message = FALSE,warning = FALSE,results ='hide',include=FALSE}
...
```
```{r change data types to the right format,echo=FALSE,message=FALSE}
...
```{r completed program relation with some numeric variables}
...
```{r completed program relation with children}
...
```{r completed program relation with laborforce status}
...
```{r completed program relation with abuse history}
...
###### Does the living have any relation with completion rate
...
```{r completed program relation with living conditions}
...
```{r completed program relation with victim abuse history}
